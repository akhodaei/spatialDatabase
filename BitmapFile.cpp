...
BOOL CBitmapFile::SaveDesktopAsFile(LPCSTR szFile, int nFileType)
{
...
BOOL CBitmapFile::SaveWindowAsFile(LPCSTR szFile, CWnd *pWnd, int nFileType)
{
	CString		strFileName, strError = "";
	CBitmap 	bitmap;
	CWindowDC	dc(pWnd);
	...
	if(nFileType == BMP_FILE)
		...
		JpegFromDib(hDIB, 80, strFileName, &strError);
...
HANDLE CBitmapFile::DDBToDIB( CBitmap& bitmap, DWORD dwCompression, CPalette* pPal ) 
{
	...
	//if( nColors > 256 )  //Remove by Shasha
	if (bi.biBitCount>8)   //Add by Shasha
		nColors = 0;
...
BOOL CBitmapFile::WriteDIB( LPCSTR szFile, HANDLE hDIB)
{
